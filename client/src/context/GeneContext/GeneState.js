...
const GeneState = (props) => {
    ...
    const FetchGenes = async() =>{
        try {
            const {data} = await axios.get("http://localhost:5000/api/v1/getgenes");
            dispatch({
    ...
    const FetchGeneById = async(id)=>{
        try {
            const {data} = await axios.get(`http://localhost:5000/api/v1/getgene/${id}`);
            dispatch({
    ...
    const AddGene = async(formData) =>{
        try {
            const {data} = await axios.post("http://localhost:5000/api/v1/creategene", formData, {
                headers:{
